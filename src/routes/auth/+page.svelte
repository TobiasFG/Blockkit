<script lang="ts">
    ...
    let feedback = $state<{ tone: "error" | "success"; text: string } | null>(
        null,
    );
    ...
    const emailValue = $derived(
        typeof (form as Record<string, unknown> | null | undefined)?.email ===
            "string"
...
<div
    class="min-h-screen overflow-hidden bg-[linear-gradient(140deg,color-mix(in_oklab,var(--background)_88%,oklch(0.97_0.02_95))_0%,var(--background)_42%,color-mix(in_oklab,var(--background)_84%,oklch(0.89_0.03_160))_100%)] text-foreground"
>
    <div class="relative min-h-screen">
        ...
        <div class="pointer-events-none absolute inset-0 overflow-hidden">
            <div
                class="absolute -left-16 top-12 h-56 w-56 rounded-full bg-amber-400/20 blur-3xl"
            ></div>
            <div
                class="absolute right-0 top-0 h-72 w-72 rounded-full bg-emerald-400/18 blur-3xl"
            ></div>
            <div
                class="absolute bottom-0 right-12 h-64 w-64 rounded-full bg-sky-500/18 blur-3xl"
            ></div>
        </div>

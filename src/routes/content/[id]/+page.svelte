<script lang="ts">
    ...
    const getRevertTargetLabel = (currentBlock: ReusableBlock) =>
        getReusableBlockPublishState(currentBlock) === "published"
            ? "published"
            : "draft";
    ...
    const hasUnsavedChanges = $derived.by(() => {
        ...
        return (
            ...
            JSON.stringify(contentDraft) !==
                JSON.stringify(loadedSnapshot.content)
        );
    });
    ...
    const primaryActionIntent = $derived(
        primaryActionState === "publish" ? "publish" : "save",
    );
    ...
    const handleRootFieldUpdate = (
        fieldKey: string,
        value: BlockValue | undefined,
    ) => {
        contentDraft = syncValidation(
            updateReusableBlockFieldValue(contentDraft, [], fieldKey, value),
        );
    };
    ...
    const resetDraft = () => {
        ...
        name = loadedSnapshot.name;
        folderId = loadedSnapshot.folderId;
        contentDraft = createEditableReusableBlockContent(
...
<main class="mx-auto max-w-[96rem] px-4 pb-10 pt-6 sm:px-6 lg:px-8">
    <header class="border-b border-border/70 pb-8">
        <div class="flex flex-wrap items-start justify-between gap-6">
            <div class="max-w-3xl space-y-3">
                ...
                <h1
                    class="text-[2.6rem] font-semibold tracking-[-0.045em] text-foreground sm:text-5xl"
                >
                    {name || block.name}
                </h1>
                <p
                ...
                >
                    Update content name, folder, and fields for this <span
                        class="font-semibold text-stone-900"
                        >{block.block_type}</span
                    > content item.
                </p>
            </div>
            <Button href="/content" variant="outline" class="rounded-full">
                Back to Content
            </Button>
        </div>
    </header>
    ...
    <form
        ...
        use:enhance={({ formElement, cancel, submitter }) => {
            ...
            if (intent === "publish") {
                if (hasUnsavedChanges) {
                    toastState.error(
                        "Save draft before publishing current content changes.",
                    );
                    cancel();
                    return;
                }
                ...
                if (result.type === "success" && result.data) {
                    ...
                    if ("block" in result.data) {
                        ...
                        loadedSnapshot = {
                            name,
                            folderId,
                            content: createEditableReusableBlockContent(
                                block.content,
                            ),
                    ...
                    if ("blockFolders" in result.data) {
                        blockFolders = result.data
                            .blockFolders as BlockFolder[];
                        if (browser) {
    ...
    >
        <input
            type="hidden"
            name="content"
            value={JSON.stringify(contentDraft)}
        />
            ...
            <section class="space-y-5">
                <div class="space-y-2">
                    ...
                    <p
                        class="max-w-[62ch] text-base leading-7 text-muted-foreground"
                    >
                        Keep content name recognizable in sidebar and page
                        insertion flows.
                    </p>
                </div>

                <div
                    class="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(16rem,0.8fr)]"
                >
                    <div class="space-y-2">
            ...
            <section class="space-y-5">
                <div class="space-y-2">
                    ...
                    <p
                        class="max-w-[62ch] text-base leading-7 text-muted-foreground"
                    >
                        Edit primitive fields inline and build nested block
                        content where this content type allows it.
                    </p>
                </div>
                        ...
                        {#each currentDefinition.fields as field (field.key)}
                            <div class="space-y-2">
                                ...
                                {:else if field.type === "boolean"}
                                    <label
                                        for={`field-${field.key}`}
                                        class="flex items-start gap-3 rounded-2xl border border-border bg-muted/40 px-4 py-3 text-sm text-foreground"
                                    >
                                        <input
                                            id={`field-${field.key}`}
                                            type="checkbox"
                ...
                >
                    ...
                    >
                        ...
                        <div class="flex items-center justify-between gap-3">
                            ...
                            >
                            <span
                                class="text-right tabular-nums text-foreground"
                                >{formatTimestamp(
                                    block.last_published_at,
                                )}</span
                            >
                        </div>
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span
                                class="text-right tabular-nums text-foreground"
                                >{formatTimestamp(block.created_at)}</span
                            >
                        </div>
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span
                                class="text-right tabular-nums text-foreground"
                                >{formatTimestamp(block.updated_at)}</span
                            >
                        </div>
                    </div>
                </section>

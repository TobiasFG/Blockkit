...
- The `Blocks` section header links to the dedicated `/blocks` library route.
...
- The `Blocks` section keeps a quick link for folder creation and routes shared-content creation to the dedicated library page.
...
- When a page editor is open, reusable block rows also expose an `Add` action and an `Insert into page` context-menu action that append a live reusable reference into the current draft.
- Reusable block folders and rows also expose a visible `Actions` trigger so the same operations remain available even when right-click/context-menu behavior is unreliable.
- On wide pointer-capable page-editor layouts, reusable block rows also act as drag sources so editors can drop them into explicit top-level page-editor insertion targets.
- Right-clicking a block folder in the sidebar opens a context menu with create-subfolder and delete actions.
- Right-clicking a reusable block in the sidebar opens a context menu with insert/delete actions, and the visible `Actions` trigger opens the same menu.
